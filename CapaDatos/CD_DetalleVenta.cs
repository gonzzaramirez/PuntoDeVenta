...
namespace CapaDatos
{
    public class CD_DetalleVenta
    {
        ...
        /*
        public List<DetalleVenta> ObtenerDetallesVenta(int idVenta, FranjaHoraria franjaHoraria)
        {
            List<DetalleVenta> lista = new List<DetalleVenta>();
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT dv.IdDetalleVenta, dv.IdProducto, dv.PrecioVenta, dv.Cantidad, dv.Subtotal, dv.FechaRegistro, p.Nombre AS NombreProducto " +
                                                           "FROM DETALLE_VENTA dv " +
                                                           "INNER JOIN PRODUCTO p ON dv.IdProducto = p.IdProducto " +
                                                           "WHERE dv.IdVenta = @IdVenta AND (CAST(dv.FechaRegistro AS TIME) BETWEEN @Inicio AND @Fin)", con))
                    {
                        cmd.CommandType = CommandType.Text;
                        cmd.Parameters.AddWithValue("@IdVenta", idVenta);
